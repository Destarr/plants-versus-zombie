...
private:
    // 默认存活时间（5秒未收集则自动消失）
    const float DEFAULT_LIFE_TIME = 5.0f;

    // 触摸相关声明
    void registerTouchListener();  // 注册触摸监听
    void unregisterTouchListener();// 注销触摸监听
    bool onTouchBegan(Touch* touch, Event* event); // 触摸开始回调
    void onTouchEnded(Touch* touch, Event* event); // 触摸结束回调

    EventListenerTouchOneByOne* m_touchListener; // 单点触摸监听器
