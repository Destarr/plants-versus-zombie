...
bool Game::init() {
    ...
    // 5.创建阳光系统
    auto sunSystem = SunSystem::create();
    this->addChild(sunSystem, 100);  
    GameDataCenter::getInstance()->setSunSystem(sunSystem);

    // 6. 创建植物放置系统
    auto plantPlacement = PlantPlacementCore::create();
    plantPlacement->setSunSystem(sunSystem);  // 设置阳光系统引用
    gameLayer->addChild(plantPlacement, 5);
